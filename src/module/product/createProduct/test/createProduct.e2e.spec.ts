...
describe('US-1 : Créer un produit - E2E', () => {
    ...
    let app: Express;
    const useExistingDb = process.env.E2E_USE_EXISTING_DB === 'true';
    ...
    beforeAll(async () => {
        ...
        dataSource = new DataSource({
            type: 'postgres',
            host: useExistingDb ? (process.env.DB_HOST ?? 'localhost') : container.getHost(),
            port: useExistingDb ? parseInt(process.env.DB_PORT ?? '5432') : container.getPort(),
            username: useExistingDb ? (process.env.DB_USER ?? 'postgres') : container.getUsername(),
            password: useExistingDb ? (process.env.DB_PW ?? 'postgres') : container.getPassword(),
            database: useExistingDb ? (process.env.DB_NAME ?? 'shopshop') : container.getDatabase(),
            logging: false,
            entities: [Product],
            synchronize: true,
    ...
    test("Scénario 6: création échouée, description contenant '@'", async () => {
        await dataSource.getRepository(Product).clear();
